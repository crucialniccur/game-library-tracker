...
if args.command == 'add-user':
    ...
    if not lib:
        print(f"Error: Library with ID {args.library_id} not found")
    else:
        game = Game(
            title=args.title,
            genre=args.genre,
    ...
    for g in games:
        status = "Completed" if g.completed is True else "Not completed"
        rating = f"{g.rating}/5" if g.rating is not None else "Not rated"
        play_time = f"{g.play_time} minutes" if g.play_time is not None else "Not played"
    ...
    if not game:
        print(f"Error: Game with ID {args.id} not found")
    else:
        if args.completed:
            game.completed = True
    ...
    game = db.get(Game, args.id)
    if not game:
        print(f"Error: No game found with ID {args.id}")
    else:
        title = game.title
        db.delete(game)
        db.commit()
